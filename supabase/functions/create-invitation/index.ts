...
Deno.serve(async req => {
  ...
  if (!body.team_id || !body.expires_at) return json(req, { error: "team_id_and_expires_at_required" }, 400);
  if (!Number.isInteger(body.max_uses ?? 1) || (body.max_uses ?? 1) < 1 || (body.max_uses ?? 1) > 50) {
    return json(req, { error: "invalid_max_uses" }, 400);
  }
  ...
  return error ? json(req, { error: "invitation_not_created" }, 400) : json(req, { invitation: data }, 201);
});
